...
import { ApiClient, BOMComponent } from './api_client';
import { ServerManager } from './server_manager';
import { listenFromIframe, sendToIframe, IframeToHost } from './iframe_comm';

// ── LCEDA Global API Types ──────────────────────────────────

// These globals are injected by the LCEDA runtime for every extension.
// They are documented at: https://prodocs.lceda.cn/cn/api/

interface SCH_ManufactureData_Static {
  getBomFile(
    document: unknown,
    fileName?: string,
    fileType?: string,
...
interface SYS_ToastMessage_Static {
  show(message: string, duration?: number): void;
}

interface SCH_Document_Static {
  getCurrent(): Promise<unknown>;
}

interface PCB_Document_Static {
  getCurrent(): Promise<unknown>;
}

// These are available at runtime in the LCEDA extension environment.
declare const eda: {
  sch_ManufactureData: SCH_ManufactureData_Static;
  sch_Document: SCH_Document_Static;
  pcb_Document: PCB_Document_Static;
  sys_IFrame: SYS_IFrame_Static;
  sys_ToastMessage: SYS_ToastMessage_Static;
};

// ── State ────────────────────────────────────────────────────

let iframePanel: HTMLIFrameElement | null = null;
...
export async function analyzeBom(): Promise<void> {
  ...
  try {
    ...
    eda.sys_ToastMessage.show(
      `BOM 分析完成: ${result.count} 种元件, ${components.length} 行`
    );
...
export async function bomHealth(): Promise<void> {
...
export async function drcCheck(): Promise<void> {
...
export async function multiAgentReview(): Promise<void> {
...
export function showAbout(): void {
  eda.sys_ToastMessage.show(
    'EDA AI 助手 v1.0 | 69条设计规则 | 10家LLM厂商 | 5 Agent审查',
    6000
  );
}
...
function setupIframeListener(): void {
  listenFromIframe(async (msg: IframeToHost) => {
    switch (msg.command) {
      case 'analyze-bom':
        await analyzeBom();
        break;
 ...
 */
function parseBomCSV(text: string): BOMComponent[] {
  ...
  const idx = (keys: string[]): number => {
    for (const key of keys) {
      const i = headers.findIndex(
        (h) => h.toLowerCase().replace(/\s/g, '') === key.toLowerCase().replace(/\s/g, '')
      );
      if (i >= 0) return i;
    }
    return -1;
